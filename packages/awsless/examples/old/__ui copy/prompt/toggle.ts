import { style, symbol } from "../../../../src/cli/style"
import { Signal, Terminal } from "../terminal"
import { br } from "../layout/basic"

export type TogglePromptOptions = {
	initial?: boolean
	active?: string
	inactive?: string
}

export const togglePrompt = (term: Terminal, label: string, options: TogglePromptOptions = {}): Promise<boolean> => {
	return new Promise(resolve => {
		const { initial = false, active = 'on', inactive = 'off' } = options
		const icon = new Signal<string>(style.info(symbol.question))
		const sep = new Signal<string>(style.placeholder(symbol.pointerSmall))
		const mid = style.placeholder('/')
		const end = new Signal<string>('')
		const activeText = new Signal(active)
		const inactiveText = new Signal(inactive)

		let value = initial

		const activate = () => {
			activeText.set(style.success.underline(active))
			inactiveText.set(style.normal(inactive))
			value = true
		}

		const deactivate = () => {
			activeText.set(style.normal(active))
			inactiveText.set(style.success.underline(inactive))
			value = false
		}

		const toggle = () => {
			!value ? activate() : deactivate()
		}

		const reset = () => {
			initial ? activate() : deactivate()
		}

		reset()

		const release = term.captureInput({
			reset,
			exit: () => {
				release()
				icon.set(style.error(symbol.error))
				sep.set(symbol.ellipsis)
				end.set(br())
				resolve(false)
			},
			submit: () => {
				release()
				icon.set(style.success(symbol.success))
				sep.set(symbol.ellipsis)
				end.set(br())
				resolve(value)
			},
			input: (chr) => {
				switch(chr) {
					case ' ': toggle(); break
					case '1': activate(); break
					case '0': deactivate(); break
				}
			},
			delete: deactivate,
			left: deactivate,
			right: activate,
			down: deactivate,
			up: activate,
		})

		term.write`${icon}  ${style.label(label)} ${sep} ${inactiveText} ${mid} ${activeText}${end}`
	})
}
