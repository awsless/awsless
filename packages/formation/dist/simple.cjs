...
var deploy = () => __async(void 0, null, function* () {
  const stack = yield import_automation.LocalWorkspace.createOrSelectStack(
    {
  ...
  yield stack.setConfig("aws:region", { value: aws.Region.EUWest1 });
  yield stack.refresh({ onOutput: console.info });
  const response = yield stack.up({
