...
var createWorkspace = (profile, region2 = "eu-west-1", timeout = 15) => {
  const stateProvider = new local_exports.StateProvider({
    dir: "./examples/resources/state"
  });
  const credentials = (0, import_credential_providers.fromIni)({ profile });
  const cloudProviders = aws_exports.createCloudProviders({
    region: region2,
    credentials,
    timeout: (0, import_duration16.minutes)(timeout)
  });
...
var createVPC = (stack2, region2, ns = "main") => {
  const group = new Node("vpc", ns);
  stack2.add(group);
  const vpc2 = new aws_exports.ec2.Vpc("vpc", {
    name: `vpc-${ns}`,
    cidrBlock: aws_exports.ec2.Peer.ipv4("10.0.0.0/16")
  });
  const privateRouteTable = new aws_exports.ec2.RouteTable("private", {
    vpcId: vpc2.id,
    name: "private"
  });
  const publicRouteTable = new aws_exports.ec2.RouteTable("public", {
    vpcId: vpc2.id,
    name: "public"
  });
  const gateway = new aws_exports.ec2.InternetGateway("gateway");
  const attachment = new aws_exports.ec2.VPCGatewayAttachment("attachment", {
    vpcId: vpc2.id,
    internetGatewayId: gateway.id
  });
  const route = new aws_exports.ec2.Route("route", {
    gatewayId: gateway.id,
    routeTableId: publicRouteTable.id,
    destination: aws_exports.ec2.Peer.anyIpv4()
  });
  group.add(vpc2, privateRouteTable, publicRouteTable, gateway, attachment, route);
  const zones = ["a", "b"];
  ...
  for (const table of tables) {
    for (const i in zones) {
      ...
      const subnet = new aws_exports.ec2.Subnet(id, {
        vpcId: vpc2.id,
        cidrBlock: aws_exports.ec2.Peer.ipv4(`10.0.${block++}.0/24`),
        availabilityZone: region2 + zones[i]
      });
      const association = new aws_exports.ec2.SubnetRouteTableAssociation(id, {
        routeTableId: table.id,
        subnetId: subnet.id
      });
      group.add(subnet, association);
      subnets2[table.identifier].push(subnet);
    }
  }
  return {
    vpc: vpc2,
...
// examples/resources/memorydb.ts
var region = "eu-west-1";
var app = new App("memorydb");
var stack = new Stack("memorydb");
app.add(stack);
var { vpc, subnets } = createVPC(stack, region);
var securityGroup = new aws_exports.ec2.SecurityGroup("memorydb-security-group", {
  name: "memorydb-security-group",
  vpcId: vpc.id,
  description: "memorydb-security-group"
});
var port = aws_exports.ec2.Port.tcp(6379);
securityGroup.addIngressRule({ port, peer: aws_exports.ec2.Peer.anyIpv4() });
securityGroup.addIngressRule({ port, peer: aws_exports.ec2.Peer.anyIpv6() });
var subnetGroup = new aws_exports.memorydb.SubnetGroup("memorydb-subnet2", {
  name: "memorydb-formation-test",
  subnetIds: subnets.private.map((s) => s.id)
});
var cluster = new aws_exports.memorydb.Cluster("memorydb", {
  name: "memorydb-formation-test",
  aclName: "open-access",
