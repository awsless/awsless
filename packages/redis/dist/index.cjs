...
var createCommand = (redis, name, args, options) => {
  return new import_ioredis.Command(name, filterArgs(args), {
...
var set = (client, key, value, options = {}) => {
  ...
  if (options.ttl instanceof import_duration.Duration) {
    args.push("PX", (0, import_duration.toSafeMilliSeconds)(options.ttl).toString());
  }
  if (options.ttl === "keep") {
    args.push("KEEPTTL");
  }
...
var incr = (client, key, value = 1) => {
  const num = (0, import_big_float.parse)(value).toString();
  return command(client, "INCRBYFLOAT", [key, num], returnEcho);
};
var decr = (client, key, value = 1) => {
  const num = (0, import_big_float.mul)(value, -1).toString();
  return command(client, "INCRBYFLOAT", [key, num], returnEcho);
};
...
var set2 = (client, key, ttl, ...fields) => {
  ...
  if (isDate) {
    ...
    args.push((0, import_duration2.toSafeMilliSeconds)(ttl).toString());
  }
  return command(
    client,
    cmd,
...
var duration = (client, key, ...fields) => {
  return command(
    ...
    (r) => r.map((v) => {
      ...
      return (0, import_duration2.milliSeconds)(v);
    })
  );
};
...
var incr2 = (client, key, field, value = 1) => {
  const num = (0, import_big_float2.parse)(value).toString();
  return command(client, "HINCRBYFLOAT", [key, field, num], returnEcho);
};
var decr2 = (client, key, field, value = 1) => {
  const num = (0, import_big_float2.mul)(value, -1).toString();
  return command(client, "HINCRBYFLOAT", [key, field, num], returnEcho);
};
...
var set4 = (client, key, ttl) => {
  ...
  if (isDate) {
    ...
    args.push((0, import_duration3.toSafeMilliSeconds)(ttl).toString());
  }
...
var duration2 = (client, key) => {
  return command(client, "PTTL", [key], (r) => {
    ...
    return (0, import_duration3.milliSeconds)(r);
  });
};
...
var returnSortedSet = (r) => (0, import_chunk2.default)(r, 2).map((entry) => returnEntry(entry));
...
var createScriptRunner = (script, keyNum = 0, readonly = false) => {
  ...
  const sha = () => {
    if (!hash) {
      hash = (0, import_node_crypto.createHash)("sha1").update(script).digest("hex");
    }
    return hash;
  };
  return (client, ...args) => {
