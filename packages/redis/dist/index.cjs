...
const createCommand = (redis, name, args, options) => {
	return new ioredis.Command(name, filterArgs(args), {
...
const createIoRedisClient = (options) => {
	const createClient = () => {
		...
		else return new ioredis.Cluster([{
			host: props.host,
			port: props.port
		}], {
...
const set$3 = (client, key, value, options = {}) => {
	...
	if (options.ttl instanceof _awsless_duration.Duration) args.push("PX", (0, _awsless_duration.toSafeMilliSeconds)(options.ttl).toString());
...
const incr$2 = (client, key, value = 1) => {
	const num = (0, _awsless_big_float.parse)(value).toString();
...
const decr$1 = (client, key, value = 1) => {
	const num = (0, _awsless_big_float.mul)(value, -1).toString();
	return command(client, "INCRBYFLOAT", [key, num], returnEcho);
};
/**
...
const set$2 = (client, key, ttl, ...fields) => {
	...
	else args.push((0, _awsless_duration.toSafeMilliSeconds)(ttl).toString());
...
const duration$1 = (client, key, ...fields) => {
	return command(client, "HPTTL", [
		...
		if (v < 0) return;
		return (0, _awsless_duration.milliSeconds)(v);
	}));
};
/**
...
const all$3 = (client, key) => {
	return command(client, "HGETALL", [key], (items) => new Map((0, chunk.default)(items, 2)));
};
const formatScanResult$2 = (result) => {
	const { cursor, items } = returnScanResult(result);
	return {
		cursor,
		items: new Map((0, chunk.default)(items, 2))
	};
};
/**
...
const duration = (client, key) => {
	return command(client, "PTTL", [key], (r) => {
		if (r < 0) return;
		return (0, _awsless_duration.milliSeconds)(r);
	});
};
/**
...
const createScriptRunner = (script, keyNum = 0, readonly = false) => {
	let hash;
	const sha = () => {
		if (!hash) hash = (0, node_crypto.createHash)("sha1").update(script).digest("hex");
		return hash;
	};
	return (client, ...args) => {
		let promise;
