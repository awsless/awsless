...
import { add, cmp, parse, string, sub } from "@awsless/big-float";
//#region src/clock.ts
...
//#endregion
//#region src/operations/create-table.ts
function validateAndCreate(store, input) {
	if (!input.TableName) throw new ValidationException("TableName is required");
	if (!input.KeySchema || input.KeySchema.length === 0) throw new ValidationException("KeySchema is required");
	if (!input.AttributeDefinitions || input.AttributeDefinitions.length === 0) throw new ValidationException("AttributeDefinitions is required");
	const hashKeys = input.KeySchema.filter((k) => k.KeyType === "HASH");
	if (hashKeys.length !== 1) throw new ValidationException("Exactly one hash key is required");
	if (input.KeySchema.filter((k) => k.KeyType === "RANGE").length > 1) throw new ValidationException("At most one range key is allowed");
	const definedAttrs = new Set(input.AttributeDefinitions.map((a) => a.AttributeName));
	for (const keyElement of input.KeySchema) if (!definedAttrs.has(keyElement.AttributeName)) throw new ValidationException(`Attribute ${keyElement.AttributeName} is specified in KeySchema but not in AttributeDefinitions`);
	if (input.GlobalSecondaryIndexes) for (const gsi of input.GlobalSecondaryIndexes) validateSecondaryIndexKeySchema(gsi.IndexName, gsi.KeySchema, definedAttrs, true);
	if (input.LocalSecondaryIndexes) {
		...
		for (const lsi of input.LocalSecondaryIndexes) {
			...
			if (!lsiHashKey || lsiHashKey.AttributeName !== tableHashKey) throw new ValidationException(`Local secondary index ${lsi.IndexName} must have the same hash key as the table`);
		}
	}
...
function createTable(store, input) {
	return { TableDescription: validateAndCreate(store, input) };
}
function validateSecondaryIndexKeySchema(indexName, keySchema, definedAttrs, isGlobal) {
	if (!keySchema.length) throw new ValidationException(`Index ${indexName} must define a key schema`);
	...
	if (hashKeys.length === 0 || hashKeys.length > maxHashKeys) throw new ValidationException(isGlobal ? `Global secondary index ${indexName} must have between 1 and 4 partition key attributes` : `Local secondary index ${indexName} must have exactly one hash key`);
	if (rangeKeys.length > maxRangeKeys) throw new ValidationException(isGlobal ? `Global secondary index ${indexName} can have at most 4 sort key attributes` : `Local secondary index ${indexName} can have at most one range key`);
	if (isGlobal && hashKeys.length + rangeKeys.length > 8) throw new ValidationException(`Global secondary index ${indexName} can have at most 8 key attributes`);
	let seenRange = false;
	for (const keyElement of keySchema) {
		if (!definedAttrs.has(keyElement.AttributeName)) throw new ValidationException(`Attribute ${keyElement.AttributeName} is specified in index ${indexName} but not in AttributeDefinitions`);
		if (keyElement.KeyType === "RANGE") seenRange = true;
		else if (seenRange) throw new ValidationException(`Index ${indexName} must list all partition key attributes before sort key attributes`);
	}
}
//#endregion
//#region src/operations/delete-table.ts
function deleteTable(store, input) {
	if (!input.TableName) throw new ValidationException("TableName is required");
	return { TableDescription: {
		...store.deleteTable(input.TableName).describe(),
		TableStatus: "DELETING"
	} };
}
//#endregion
//#region src/operations/describe-table.ts
function describeTable(store, input) {
	if (!input.TableName) throw new ValidationException("TableName is required");
	return { Table: store.getTable(input.TableName).describe() };
}
//#endregion
//#region src/operations/list-tables.ts
...
//#endregion
//#region src/expressions/path.ts
	...
	while (i < path.length) {
		...
		if (char === ".") {
			if (current) {
				segments.push({
					type: "attribute",
					value: resolveAttributeName(current, attributeNames)
				});
				current = "";
			}
			i++;
...
function getValueAtPath(item, segments) {
	...
	for (const segment of segments) {
		...
		if (segment.type === "attribute") {
			if ("M" in current) current = current.M[segment.value];
			else return;
		} else if (segment.type === "index") {
			if ("L" in current) current = current.L[segment.value];
			else return;
		}
	}
	return current;
...
function setValueAtPath(item, segments, value) {
	...
	for (let i = 0; i < segments.length - 1; i++) {
		...
		if (segment.type === "attribute") {
			if (!("M" in current)) return;
			const map = current.M;
			const attrName = segment.value;
			if (!map[attrName]) {
				if (nextSegment.type === "attribute") map[attrName] = { M: {} };
				else map[attrName] = { L: [] };
			}
			...
			if (!list[idx]) {
				if (nextSegment.type === "attribute") list[idx] = { M: {} };
				else list[idx] = { L: [] };
			}
			current = list[idx];
		}
...
function deleteValueAtPath(item, segments) {
	...
	for (let i = 0; i < segments.length - 1; i++) {
		const segment = segments[i];
		if (segment.type === "attribute") {
			if (!("M" in current)) return false;
			const map = current.M;
			if (!map[segment.value]) return false;
			current = map[segment.value];
		} else if (segment.type === "index") {
			if (!("L" in current)) return false;
			const list = current.L;
			if (list[segment.value] === void 0) return false;
	...
	if (lastSegment.type === "attribute") {
		...
		if ("L" in current) {
			current.L.splice(lastSegment.value, 1);
			return true;
		}
	}
	return false;
...
//#endregion
//#region src/expressions/condition.ts
function tokenize(expression) {
	...
	while (i < expression.length) {
		...
		if (/[a-zA-Z_]/.test(char)) {
			...
			if (keywords[upper]) tokens.push({
				type: keywords[upper],
				value: upper
			});
			else if (functions.includes(word)) tokens.push({
				type: "FUNCTION",
				value: word
			});
			else tokens.push({
		...
		if (char === "[") {
			...
			value += "]";
			i++;
			if (tokens.length > 0 && tokens[tokens.length - 1].type === "PATH") tokens[tokens.length - 1].value += value;
			continue;
		}
		if (char === ".") {
...
function evaluateCondition(expression, item, context) {
	...
	function consume(type) {
		...
		if (!token) throw new ValidationException("Unexpected end of expression");
		if (type && token.type !== type) throw new ValidationException(`Expected ${type} but got ${token.type}`);
		pos++;
		return token;
	}
	...
	function parsePrimary() {
		...
		if (token.type === "FUNCTION") return parseFunction();
		if (token.type === "PATH" || token.type === "VALUE") return parseComparison();
	...
	function parseSizeValue() {
		...
		consume("RPAREN");
		const value = getValueAtPath(item, parsePath(pathToken.value, context.expressionAttributeNames));
		let size = 0;
		if (value) {
	...
	function parseFunction() {
		...
		if (funcName === "attribute_exists") {
			const pathToken = consume("PATH");
			consume("RPAREN");
			return getValueAtPath(item, parsePath(pathToken.value, context.expressionAttributeNames)) !== void 0;
		}
		...
		if (funcName === "attribute_type") {
			...
			if (!value || !expectedType || !("S" in expectedType)) return false;
			const typeMap = {
				S: "S",
				N: "N",
				B: "B",
		...
		if (funcName === "begins_with") {
			...
			if ("S" in value && "S" in prefix) return value.S.startsWith(prefix.S);
			if ("B" in value && "B" in prefix) return value.B.startsWith(prefix.B);
			return false;
		}
		if (funcName === "contains") {
			...
			if ("S" in value && "S" in operand) return value.S.includes(operand.S);
			if ("SS" in value && "S" in operand) return value.SS.includes(operand.S);
			if ("NS" in value && "N" in operand) return value.NS.includes(operand.N);
			if ("BS" in value && "B" in operand) return value.BS.includes(operand.B);
			if ("L" in value) return value.L.some((v) => compareValues$1(v, operand) === 0);
			return false;
		}
		if (funcName === "size") {
			const pathToken = consume("PATH");
			...
			if (nextToken?.type === "COMPARATOR") {
				consume("COMPARATOR");
				let rightNum;
				if (current()?.type === "FUNCTION" && current()?.value === "size") rightNum = parseSizeValue();
				else {
					const rightValue = resolveOperand(consume());
					if (!rightValue || !("N" in rightValue)) throw new ValidationException("Size comparison requires numeric operand");
	...
	function parseComparison() {
		const leftValue = resolveOperand(consume());
		const nextToken = current();
		if (nextToken?.type === "COMPARATOR") {
			const op = consume("COMPARATOR").value;
			return compare(leftValue, resolveOperand(consume()), op);
		}
		if (nextToken?.type === "BETWEEN") {
			consume("BETWEEN");
			const lowValue = resolveOperand(consume());
			consume("AND");
			const highValue = resolveOperand(consume());
			if (!leftValue || !lowValue || !highValue) return false;
			return compareValues$1(leftValue, lowValue) >= 0 && compareValues$1(leftValue, highValue) <= 0;
		}
		if (nextToken?.type === "IN") {
			...
			return values.some((v) => v && compareValues$1(leftValue, v) === 0);
		}
		return leftValue !== void 0;
	}
	function resolveOperand(token) {
		if (token.type === "VALUE") return resolveValue(token.value);
		if (token.type === "PATH") return getValueAtPath(item, parsePath(token.value, context.expressionAttributeNames));
	}
	function resolveValue(ref) {
		if (ref.startsWith(":") && context.expressionAttributeValues) return context.expressionAttributeValues[ref];
	}
	function compare(left, right, op) {
		...
		const cmp = compareValues$1(left, right);
		switch (op) {
			case "=": return cmp === 0;
			case "<>": return cmp !== 0;
			case "<": return cmp < 0;
...
function compareValues$1(a, b) {
	...
	if ("BOOL" in a && "BOOL" in b) return Number(a.BOOL) - Number(b.BOOL);
	if ("NULL" in a && "NULL" in b) return 0;
	const aStr = JSON.stringify(a);
	const bStr = JSON.stringify(b);
	return aStr.localeCompare(bStr);
}
//#endregion
//#region src/store/item.ts
function extractKey(item, keySchema) {
	const key = {};
	for (const element of keySchema) {
		const value = item[element.AttributeName];
		if (value) key[element.AttributeName] = value;
	}
	return key;
}
function serializeKey(key, keySchema) {
	...
	for (const element of keySchema) {
		...
		if (value) parts.push(serializeAttributeValue(value));
	}
	return parts.join("#");
}
...
function getHashKey(keySchema) {
	...
	if (!hash) throw new Error("No hash key found");
...
function getRangeKey(keySchema) {
	return keySchema.find((k) => k.KeyType === "RANGE")?.AttributeName;
}
...
function mergeKeySchemas(...schemas) {
	const merged = [];
	for (const schema of schemas) for (const element of schema) if (!merged.some((existing) => existing.AttributeName === element.AttributeName)) merged.push(element);
	return merged;
}
...
//#endregion
//#region src/expressions/key-condition.ts
...
function parseKeyCondition(expression, keySchema, context) {
	...
	function resolveName(name) {
		if (name.startsWith("#")) {
			...
			if (resolved === void 0) throw new ValidationException(`Expression attribute name ${name} is not defined`);
			return resolved;
		}
		return name;
	}
	function resolveValue(ref) {
		if (ref.startsWith(":")) {
			...
			if (value === void 0) throw new ValidationException(`Expression attribute value ${ref} is not defined`);
			return value;
		}
		throw new ValidationException(`Invalid value reference: ${ref}`);
	}
	const parts = flattenKeyConditions(stripOuterParens(expression));
	...
	for (const part of parts) {
		...
		if (beginsWithMatch) {
			...
			if (rangeKeyNames.includes(attrName)) rangeConditions.set(attrName, {
				operator: "begins_with",
				value
			});
			else throw new ValidationException(`begins_with can only be used on sort key`);
			continue;
		}
		...
		if (betweenMatch) {
			...
			else throw new ValidationException(`BETWEEN can only be used on sort key`);
			continue;
		}
		...
		if (comparisonMatch) {
			...
			if (hashKeyNames.includes(attrName)) {
				if (operator !== "=") throw new ValidationException(`Partition key condition must use = operator`);
				hashConditions.set(attrName, value);
			} else if (rangeKeyNames.includes(attrName)) rangeConditions.set(attrName, {
				operator,
				value
			});
			else throw new ValidationException(`Key condition references unknown attribute: ${attrName}`);
			continue;
		}
		throw new ValidationException(`Invalid key condition expression: ${trimmed}`);
	}
	for (const key of hashKeyNames) if (!hashConditions.has(key)) throw new ValidationException(`Key condition must specify equality for partition key ${key}`);
	let lastSortKeyIndex = -1;
	for (const [index, key] of rangeKeyNames.entries()) {
		if (!rangeConditions.get(key)) {
			if (lastSortKeyIndex !== -1) throw new ValidationException(`Sort key conditions must reference a contiguous prefix of the key schema`);
			continue;
		}
		if (index > 0 && !rangeConditions.has(rangeKeyNames[index - 1])) throw new ValidationException(`Sort key conditions must reference a contiguous prefix of the key schema`);
		if (lastSortKeyIndex !== -1) {
			const previous = rangeConditions.get(rangeKeyNames[lastSortKeyIndex]);
			if (previous && previous.operator !== "=") throw new ValidationException(`Only the last sort key condition can use a range operator`);
		}
...
function matchesKeyCondition(item, condition) {
	for (const hashCondition of condition.hashConditions) {
		...
		if (!itemHashValue || !attributeEquals(itemHashValue, hashCondition.value)) return false;
	}
	for (const rangeCondition of condition.rangeConditions) {
		const itemRangeValue = item[rangeCondition.key];
		if (!itemRangeValue) return false;
		if (!matchesRangeCondition(itemRangeValue, rangeCondition)) return false;
	}
	return true;
}
...
function matchesRangeCondition(value, condition) {
	...
	switch (condition.operator) {
		...
		case "begins_with":
			if ("S" in value && "S" in condition.value) return value.S.startsWith(condition.value.S);
			if ("B" in value && "B" in condition.value) return value.B.startsWith(condition.value.B);
			return false;
		default: return false;
	}
}
...
//#endregion
//#region src/expressions/projection.ts
function applyProjection(item, projectionExpression, expressionAttributeNames) {
	if (!projectionExpression || projectionExpression.trim() === "") return item;
	...
	for (const path of paths) {
		...
		if (value !== void 0) setValueAtPath(result, segments, value);
	}
	return result;
}
//#endregion
//#region src/expressions/update.ts
	...
	while (remaining.length > 0) {
		...
		if (setMatch) {
			...
			for (const item of items) {
				...
				if (eqIdx === -1) throw new ValidationException(`Invalid SET action: ${item}`);
			...
			const { items, rest } = parseActionList(remaining);
			for (const item of items) actions.push({
				type: "REMOVE",
				path: item.trim()
			});
			remaining = rest;
			...
			for (const item of items) {
				...
				if (parts.length < 2) throw new ValidationException(`Invalid ADD action: ${item}`);
				actions.push({
					type: "ADD",
					path: parts[0],
			...
			for (const item of items) {
				const parts = item.trim().split(/\s+/);
				if (parts.length < 2) throw new ValidationException(`Invalid DELETE action: ${item}`);
				actions.push({
					type: "DELETE",
					path: parts[0],
					value: parts.slice(1).join(" ")
...
function parseActionList(expression) {
	...
	while (i < expression.length) {
		const char = expression[i];
		for (const keyword of stopKeywords) if (expression.slice(i).toUpperCase().startsWith(keyword + " ") && depth === 0 && current.trim()) {
			items.push(current.trim());
			return {
				items,
				rest: expression.slice(i)
			};
		}
		if (char === "(") {
			depth++;
...
function applyUpdateExpression(item, expression, context) {
	if (!expression || expression.trim() === "") return item;
	const result = JSON.parse(JSON.stringify(item));
	const actions = parseUpdateExpression(expression);
	const removeActions = [];
	const otherActions = [];
	for (const action of actions) if (action.type === "REMOVE") removeActions.push(action);
	else otherActions.push(action);
	for (const action of otherActions) switch (action.type) {
		case "SET":
		...
		case "ADD":
			...
			break;
		case "DELETE": applyDeleteAction(result, action, context);
	}
	removeActions.sort((a, b) => {
		...
		let aLastIdx;
		let bLastIdx;
		for (const seg of aSegments) if (seg.type === "index") aLastIdx = seg.value;
		for (const seg of bSegments) if (seg.type === "index") bLastIdx = seg.value;
		if (aLastIdx !== void 0 && bLastIdx !== void 0) return bLastIdx - aLastIdx;
		return 0;
	});
	for (const action of removeActions) applyRemoveAction(result, action, context);
	return result;
}
function resolveOperand(item, operand, context) {
	operand = operand.trim();
	if (operand.startsWith(":")) return context.expressionAttributeValues?.[operand];
	...
	if (listAppendMatch) {
		...
		if (list1 && "L" in list1 && list2 && "L" in list2) return { L: [...list1.L, ...list2.L] };
		return list1 && "L" in list1 ? list1 : list2;
	}
	return getValueAtPath(item, parsePath(operand, context.expressionAttributeNames));
}
function applySetAction(item, action, context) {
	...
	if (action.operation === "if_not_exists") {
		...
		if (existingValue !== void 0) value = existingValue;
		else value = resolveOperand(item, action.operands[1], context);
		...
		if (list1 && "L" in list1 && list2 && "L" in list2) value = { L: [...list1.L, ...list2.L] };
		else if (list1 && "L" in list1) value = list1;
		else if (list2 && "L" in list2) value = list2;
		...
		if (left && "N" in left && right && "N" in right) value = { N: string(add(parse(left.N), parse(right.N))) };
	} else if (action.operation === "minus") {
		const left = resolveOperand(item, action.operands[0], context);
		const right = resolveOperand(item, action.operands[1], context);
		if (left && "N" in left && right && "N" in right) value = { N: string(sub(parse(left.N), parse(right.N))) };
	} else value = resolveOperand(item, action.value, context);
	if (value !== void 0) setValueAtPath(item, segments, value);
}
function applyRemoveAction(item, action, context) {
	deleteValueAtPath(item, parsePath(action.path, context.expressionAttributeNames));
}
function applyAddAction(item, action, context) {
	...
	if (!addValue) return;
	if ("N" in addValue) {
		if (existingValue && "N" in existingValue) setValueAtPath(item, segments, { N: string(add(parse(existingValue.N), parse(addValue.N))) });
		else if (!existingValue) setValueAtPath(item, segments, addValue);
...
function applyDeleteAction(item, action, context) {
	...
	const existingValue = getValueAtPath(item, segments);
	if (!deleteValue || !existingValue) return;
	if ("SS" in deleteValue && "SS" in existingValue) {
		...
		if (remaining.length > 0) setValueAtPath(item, segments, { SS: remaining });
		else deleteValueAtPath(item, segments);
		...
		if (remaining.length > 0) setValueAtPath(item, segments, { NS: remaining });
		else deleteValueAtPath(item, segments);
		...
		if (remaining.length > 0) setValueAtPath(item, segments, { BS: remaining });
		else deleteValueAtPath(item, segments);
	}
}
//#endregion
//#region src/operations/put-item.ts
function putItem(store, input) {
	if (!input.TableName) throw new ValidationException("TableName is required");
	if (!input.Item) throw new ValidationException("Item is required");
	const table = store.getTable(input.TableName);
	const hashKey = table.getHashKeyName();
	if (!input.Item[hashKey]) throw new ValidationException(`Missing the key ${hashKey} in the item`);
	const rangeKey = table.getRangeKeyName();
	if (rangeKey && !input.Item[rangeKey]) throw new ValidationException(`Missing the key ${rangeKey} in the item`);
	const existingItem = table.getItem(input.Item);
	if (input.ConditionExpression) {
		if (!evaluateCondition(input.ConditionExpression, existingItem || {}, {
			expressionAttributeNames: input.ExpressionAttributeNames,
			expressionAttributeValues: input.ExpressionAttributeValues
		})) {
			if (input.ReturnValuesOnConditionCheckFailure === "ALL_OLD" && existingItem) throw new ConditionalCheckFailedException("The conditional request failed", existingItem);
			throw new ConditionalCheckFailedException("The conditional request failed");
		}
	}
	const oldItem = table.putItem(input.Item);
	const output = {};
	if (input.ReturnValues === "ALL_OLD" && oldItem) output.Attributes = oldItem;
	if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") output.ConsumedCapacity = {
		TableName: input.TableName,
		CapacityUnits: 1,
...
//#endregion
//#region src/operations/get-item.ts
function getItem(store, input) {
	if (!input.TableName) throw new ValidationException("TableName is required");
	if (!input.Key) throw new ValidationException("Key is required");
	const table = store.getTable(input.TableName);
	const hashKey = table.getHashKeyName();
	if (!input.Key[hashKey]) throw new ValidationException(`Missing the key ${hashKey} in the key`);
	const rangeKey = table.getRangeKeyName();
	if (rangeKey && !input.Key[rangeKey]) throw new ValidationException(`Missing the key ${rangeKey} in the key`);
	let item = table.getItem(input.Key);
	if (item && input.ProjectionExpression) item = applyProjection(item, input.ProjectionExpression, input.ExpressionAttributeNames);
	const output = {};
...
//#endregion
//#region src/operations/delete-item.ts
...
//#endregion
//#region src/operations/update-item.ts
function updateItem(store, input) {
	...
	if (input.UpdateExpression) item = applyUpdateExpression(item, input.UpdateExpression, {
		expressionAttributeNames: input.ExpressionAttributeNames,
		expressionAttributeValues: input.ExpressionAttributeValues
	});
	for (const [key, value] of Object.entries(input.Key)) item[key] = value;
	table.updateItem(input.Key, item);
	const output = {};
	switch (input.ReturnValues) {
		case "ALL_OLD":
			if (existingItem) output.Attributes = existingItem;
			break;
		case "ALL_NEW":
			output.Attributes = item;
			break;
		case "UPDATED_OLD":
			if (existingItem && input.UpdateExpression) output.Attributes = getUpdatedAttributes(existingItem, item);
			break;
		case "UPDATED_NEW": if (input.UpdateExpression) output.Attributes = getUpdatedAttributes(existingItem || {}, item);
	}
	if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") output.ConsumedCapacity = {
		TableName: input.TableName,
		CapacityUnits: 1,
...
function getUpdatedAttributes(oldItem, newItem) {
	...
	for (const [key, newValue] of Object.entries(newItem)) {
		...
		if (!oldValue || JSON.stringify(oldValue) !== JSON.stringify(newValue)) updated[key] = newValue;
	}
	return updated;
}
//#endregion
//#region src/operations/query.ts
function query(store, input) {
	if (!input.TableName) throw new ValidationException("TableName is required");
	if (!input.KeyConditionExpression) throw new ValidationException("KeyConditionExpression is required");
	const table = store.getTable(input.TableName);
	let keySchema = table.keySchema;
	if (input.IndexName) {
		const indexKeySchema = table.getIndexKeySchema(input.IndexName);
		if (!indexKeySchema) throw new ResourceNotFoundException(`Index ${input.IndexName} not found`);
	...
	if (keyCondition.rangeCondition) items = items.filter((item) => matchesKeyCondition(item, keyCondition));
	const scannedCount = items.length;
	if (input.FilterExpression) items = items.filter((item) => evaluateCondition(input.FilterExpression, item, {
		expressionAttributeNames: input.ExpressionAttributeNames,
		expressionAttributeValues: input.ExpressionAttributeValues
	}));
	if (input.Limit && items.length > input.Limit) {
		...
		if (items.length > 0) {
			...
			if (input.IndexName) {
				const indexKeySchema = table.getIndexKeySchema(input.IndexName);
				if (indexKeySchema) lastEvaluatedKey = extractKey(lastItem, mergeKeySchemas(indexKeySchema, table.keySchema));
			}
		}
	}
	if (input.ProjectionExpression) items = items.map((item) => applyProjection(item, input.ProjectionExpression, input.ExpressionAttributeNames));
	const output = {
		Count: items.length,
		ScannedCount: scannedCount
	};
	if (input.Select !== "COUNT") output.Items = items;
	if (lastEvaluatedKey && Object.keys(lastEvaluatedKey).length > 0) output.LastEvaluatedKey = lastEvaluatedKey;
	if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") output.ConsumedCapacity = {
		TableName: input.TableName,
		CapacityUnits: Math.max(.5, scannedCount * .5),
		ReadCapacityUnits: Math.max(.5, scannedCount * .5)
	};
	return output;
}
//#endregion
//#region src/operations/scan.ts
function scan(store, input) {
	...
	let lastEvaluatedKey;
	if (input.IndexName) {
		if (!table.hasIndex(input.IndexName)) throw new ResourceNotFoundException(`Index ${input.IndexName} not found`);
...
//#endregion
//#region src/operations/batch-get-item.ts
const MAX_BATCH_GET_ITEMS = 100;
function batchGetItem(store, input) {
	if (!input.RequestItems) throw new ValidationException("RequestItems is required");
	let totalKeys = 0;
	for (const tableRequest of Object.values(input.RequestItems)) totalKeys += tableRequest.Keys.length;
	if (totalKeys > MAX_BATCH_GET_ITEMS) throw new ValidationException(`Too many items requested for the BatchGetItem call. Max is ${MAX_BATCH_GET_ITEMS}`);
	const responses = {};
	...
	for (const [tableName, tableRequest] of Object.entries(input.RequestItems)) {
		...
		for (const key of tableRequest.Keys) {
			...
			if (item) {
				if (tableRequest.ProjectionExpression) item = applyProjection(item, tableRequest.ProjectionExpression, tableRequest.ExpressionAttributeNames);
				items.push(item);
				capacityUnits += .5;
			}
		}
		if (items.length > 0) responses[tableName] = items;
		if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") consumedCapacity.push({
			TableName: tableName,
	...
	const output = {};
	if (Object.keys(responses).length > 0) output.Responses = responses;
	if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") output.ConsumedCapacity = consumedCapacity;
	return output;
}
//#endregion
//#region src/operations/batch-write-item.ts
const MAX_BATCH_WRITE_ITEMS = 25;
function batchWriteItem(store, input) {
	if (!input.RequestItems) throw new ValidationException("RequestItems is required");
	let totalItems = 0;
	for (const tableRequests of Object.values(input.RequestItems)) totalItems += tableRequests.length;
	if (totalItems > MAX_BATCH_WRITE_ITEMS) throw new ValidationException(`Too many items requested for the BatchWriteItem call. Max is ${MAX_BATCH_WRITE_ITEMS}`);
	const consumedCapacity = [];
	for (const [tableName, requests] of Object.entries(input.RequestItems)) {
		const table = store.getTable(tableName);
		let capacityUnits = 0;
		for (const request of requests) if ("PutRequest" in request) {
			const item = request.PutRequest.Item;
			const hashKey = table.getHashKeyName();
			if (!item[hashKey]) throw new ValidationException(`Missing the key ${hashKey} in the item`);
			const rangeKey = table.getRangeKeyName();
			if (rangeKey && !item[rangeKey]) throw new ValidationException(`Missing the key ${rangeKey} in the item`);
			table.putItem(item);
			capacityUnits += 1;
		} else if ("DeleteRequest" in request) {
			const key = request.DeleteRequest.Key;
			const hashKey = table.getHashKeyName();
			if (!key[hashKey]) throw new ValidationException(`Missing the key ${hashKey} in the key`);
			const rangeKey = table.getRangeKeyName();
			if (rangeKey && !key[rangeKey]) throw new ValidationException(`Missing the key ${rangeKey} in the key`);
...
//#endregion
//#region src/operations/transact-get-items.ts
const MAX_TRANSACT_ITEMS$1 = 100;
function transactGetItems(store, input) {
	if (!input.TransactItems) throw new ValidationException("TransactItems is required");
	if (input.TransactItems.length > MAX_TRANSACT_ITEMS$1) throw new ValidationException(`Too many items in the TransactGetItems call. Max is ${MAX_TRANSACT_ITEMS$1}`);
	const responses = [];
	const consumedCapacity = /* @__PURE__ */ new Map();
	for (const transactItem of input.TransactItems) {
		const { Get: getRequest } = transactItem;
		if (!getRequest.TableName) throw new ValidationException("TableName is required in Get request");
		if (!getRequest.Key) throw new ValidationException("Key is required in Get request");
		let item = store.getTable(getRequest.TableName).getItem(getRequest.Key);
		if (item && getRequest.ProjectionExpression) item = applyProjection(item, getRequest.ProjectionExpression, getRequest.ExpressionAttributeNames);
	...
	const output = { Responses: responses };
	if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") output.ConsumedCapacity = Array.from(consumedCapacity.entries()).map(([tableName, units]) => ({
		TableName: tableName,
		CapacityUnits: units,
...
//#endregion
//#region src/operations/transact-write-items.ts
const MAX_TRANSACT_ITEMS = 100;
const idempotencyTokens = /* @__PURE__ */ new Map();
const IDEMPOTENCY_WINDOW_MS = 6e5;
function transactWriteItems(store, input) {
	if (!input.TransactItems) throw new ValidationException("TransactItems is required");
	if (input.TransactItems.length > MAX_TRANSACT_ITEMS) throw new ValidationException(`Too many items in the TransactWriteItems call. Max is ${MAX_TRANSACT_ITEMS}`);
	if (input.ClientRequestToken) {
		const cached = idempotencyTokens.get(input.ClientRequestToken);
		if (cached) {
			if (Date.now() - cached.timestamp < IDEMPOTENCY_WINDOW_MS) return cached.result;
	...
	for (const transactItem of input.TransactItems) {
		...
		} else throw new ValidationException("Invalid transaction item");
		const table = store.getTable(tableName);
		const keyString = `${tableName}#${serializeKey(key, table.keySchema)}`;
		if (itemKeys.has(keyString)) throw new ValidationException("Transaction request cannot include multiple operations on one item");
	...
	for (const transactItem of input.TransactItems) {
		...
		if (reason.Code !== "None") hasCancellation = true;
	}
	if (hasCancellation) throw new TransactionCanceledException("Transaction cancelled, please refer cancance reasons for specific reasons", cancellationReasons);
	const consumedCapacity = /* @__PURE__ */ new Map();
	for (const transactItem of input.TransactItems) executeTransactionItem(store, transactItem, consumedCapacity);
	const output = {};
	if (input.ReturnConsumedCapacity && input.ReturnConsumedCapacity !== "NONE") output.ConsumedCapacity = Array.from(consumedCapacity.entries()).map(([tableName, units]) => ({
		TableName: tableName,
		CapacityUnits: units,
	...
	if (input.ClientRequestToken) idempotencyTokens.set(input.ClientRequestToken, {
		timestamp: Date.now(),
		result: output
	});
	return output;
...
function validateTransactionItem(store, transactItem) {
	try {
		if ("ConditionCheck" in transactItem) {
			...
			const existingItem = store.getTable(check.TableName).getItem(check.Key);
			if (!evaluateCondition(check.ConditionExpression, existingItem || {}, {
			...
			if (put.ConditionExpression) {
				...
				if (!evaluateCondition(put.ConditionExpression, existingItem || {}, {
			...
			if (del.ConditionExpression) {
				const existingItem = store.getTable(del.TableName).getItem(del.Key);
				if (!evaluateCondition(del.ConditionExpression, existingItem || {}, {
			...
			if (update.ConditionExpression) {
				const existingItem = store.getTable(update.TableName).getItem(update.Key);
				if (!evaluateCondition(update.ConditionExpression, existingItem || {}, {
...
function executeTransactionItem(store, transactItem, consumedCapacity) {
	...
	if ("Put" in transactItem) {
		const { Put: put } = transactItem;
		store.getTable(put.TableName).putItem(put.Item);
	...
	if ("Delete" in transactItem) {
		const { Delete: del } = transactItem;
		store.getTable(del.TableName).deleteItem(del.Key);
	...
	if ("Update" in transactItem) {
		...
		for (const [key, value] of Object.entries(update.Key)) item[key] = value;
...
const getRequestContext = () => current;
//#endregion
...
function formatError(error) {
	if (error instanceof DynamoDBError) return {
...
async function handleRequest(store, method, target, getBody, context) {
	...
	try {
		...
		return {
			...formatError(new SerializationException("Could not parse request body")),
			requestId
		};
	}
	try {
...
function createBunServer(store, port, hostname) {
	const server = Bun.serve({
		...
		hostname,
		async fetch(req) {
			const result = await handleRequest(store, req.method, req.headers.get("X-Amz-Target"), () => req.text(), req.headers.get("x-awsless-trace") ?? void 0);
...
function createNodeServer(store, port, hostname) {
	return new Promise((resolve, reject) => {
		const server = createServer(async (req, res) => {
			...
			const result = await handleRequest(store, req.method ?? "GET", req.headers["x-amz-target"], getBody, typeof req.headers["x-awsless-trace"] === "string" ? req.headers["x-awsless-trace"] : void 0);
...
function createServer$1(store, port, hostname = "127.0.0.1") {
	if (isBun) return createBunServer(store, port, hostname);
	return createNodeServer(store, port, hostname);
}
//#endregion
//#region src/store/table.ts
...
var Table = class {
	...
	constructor(options, region = "us-east-1") {
		...
		if (options.globalSecondaryIndexes) for (const gsi of options.globalSecondaryIndexes) this.globalSecondaryIndexes.set(gsi.IndexName, {
			keySchema: gsi.KeySchema,
			projection: gsi.Projection,
			provisionedThroughput: gsi.ProvisionedThroughput,
			items: /* @__PURE__ */ new Map()
		});
		if (options.localSecondaryIndexes) for (const lsi of options.localSecondaryIndexes) this.localSecondaryIndexes.set(lsi.IndexName, {
	...
	getTtlAttributeName() {
		if (this.ttlSpecification?.Enabled) return this.ttlSpecification.AttributeName;
	}
	...
	describe() {
		...
		if (this.billingMode === "PROVISIONED" && this.provisionedThroughput) desc.ProvisionedThroughput = {
		...
		else desc.BillingModeSummary = { BillingMode: "PAY_PER_REQUEST" };
		if (this.globalSecondaryIndexes.size > 0) desc.GlobalSecondaryIndexes = this.describeGlobalSecondaryIndexes();
		if (this.localSecondaryIndexes.size > 0) desc.LocalSecondaryIndexes = this.describeLocalSecondaryIndexes();
	...
	describeGlobalSecondaryIndexes() {
		...
		for (const [name, data] of this.globalSecondaryIndexes) {
			let itemCount = 0;
			for (const keys of data.items.values()) itemCount += keys.size;
			indexes.push({
				IndexName: name,
	...
	estimateTableSize() {
		let size = 0;
		for (const item of this.items.values()) size += estimateItemSize(item);
		return size;
	}
	getItem(key) {
	...
	updateIndexes(newItem, oldItem) {
		if (oldItem) this.removeFromIndexes(oldItem);
		...
		for (const [, indexData] of this.globalSecondaryIndexes) {
			...
			if (indexKey) {
				...
				if (keys) {
					...
					if (keys.size === 0) indexData.items.delete(indexKey);
				}
			}
		}
		for (const [, indexData] of this.localSecondaryIndexes) {
	...
	buildIndexKey(item, keySchema) {
		if (!hasCompleteKey(item, keySchema)) return null;
	...
	scan(limit, exclusiveStartKey) {
		...
		allItems.sort((a, b) => {
			...
			if (rangeAttr) {
				...
				if (rangeA && rangeB) return this.compareAttributes(rangeA, rangeB);
			}
			return 0;
		});
		let startIdx = 0;
		if (exclusiveStartKey) {
			...
			startIdx = allItems.findIndex((item) => serializeKey(extractKey(item, this.keySchema), this.keySchema) === startKey);
	...
	queryIndex(indexName, hashValue, options) {
		...
		if (!indexData) throw new Error(`Index ${indexName} not found`);
		const result = this.queryBySchema(indexData.keySchema, hashValue, mergeKeySchemas(indexData.keySchema, this.keySchema), true, options);
		return {
			items: result.items,
			lastEvaluatedKey: result.lastEvaluatedKey,
	...
	scanIndex(indexName, limit, exclusiveStartKey) {
		...
		if (!indexData) throw new Error(`Index ${indexName} not found`);
		const matchingItems = [];
		for (const item of this.items.values()) if (hasCompleteKey(item, indexData.keySchema)) matchingItems.push(deepClone(item));
		...
		if (exclusiveStartKey) {
			...
			startIdx = matchingItems.findIndex((item) => serializeKey(extractKey(item, paginationKeySchema), paginationKeySchema) === startKey);
			if (startIdx !== -1) startIdx++;
			else startIdx = 0;
		}
		...
		let lastEvaluatedKey;
		if (hasMore && lastItem) lastEvaluatedKey = extractKey(lastItem, mergeKeySchemas(indexData.keySchema, this.keySchema));
		return {
			items: sliced,
			lastEvaluatedKey,
			indexKeySchema: indexData.keySchema
	...
	compareByKeySchema(a, b, keySchema, scanIndexForward = true) {
		for (const attributeName of [...getHashKeys(keySchema), ...getRangeKeys(keySchema)]) {
			...
			if (cmp !== 0) return scanIndexForward ? cmp : -cmp;
		}
		return 0;
	}
	queryBySchema(keySchema, hashValue, paginationKeySchema, requireCompleteKey, options) {
		...
		for (const item of this.items.values()) {
			if (requireCompleteKey && !hasCompleteKey(item, keySchema)) continue;
			if (hashAttrs.every((attr) => {
		...
		if (rangeAttrs.length > 0) matchingItems.sort((a, b) => this.compareByKeySchema(a, b, keySchema, options?.scanIndexForward !== false));
		let startIdx = 0;
	...
	clear() {
		this.items.clear();
		for (const index of this.globalSecondaryIndexes.values()) index.items.clear();
		for (const index of this.localSecondaryIndexes.values()) index.items.clear();
	}
	...
	emitStreamRecord(eventName, keys, oldImage, newImage) {
		if (!this.streamSpecification?.StreamEnabled || this.streamCallbacks.size === 0) return;
		...
		if (viewType === "NEW_IMAGE" || viewType === "NEW_AND_OLD_IMAGES") {
			if (newImage) record.dynamodb.NewImage = deepClone(newImage);
		}
		if (viewType === "OLD_IMAGE" || viewType === "NEW_AND_OLD_IMAGES") {
			if (oldImage) record.dynamodb.OldImage = deepClone(oldImage);
		}
		const context = getRequestContext();
		for (const callback of this.streamCallbacks) try {
	...
	expireTtlItems(currentTimeSeconds) {
		...
		for (const [keyString, item] of this.items) {
			...
			if (ttlValue && "N" in ttlValue) {
				if (parseInt(ttlValue.N, 10) <= currentTimeSeconds) {
...
var TableStore = class {
	...
	createTable(input) {
		if (this.tables.has(input.TableName)) throw new ResourceInUseException(`Table already exists: ${input.TableName}`);
	...
	getTable(tableName) {
		...
		if (!table) throw new ResourceNotFoundException(`Requested resource not found: Table: ${tableName} not found`);
		return table;
	}
	hasTable(tableName) {
	...
	listTables(exclusiveStartTableName, limit) {
		...
		return {
			tableNames,
			lastEvaluatedTableName: (limit ? startIdx + tableNames.length < allNames.length : false) ? tableNames[tableNames.length - 1] : void 0
		};
	}
	clear() {
		this.tables.clear();
	...
	expireTtlItems(currentTimeSeconds) {
		for (const table of this.tables.values()) table.expireTtlItems(currentTimeSeconds);
	}
};
//#endregion
//#region src/dynamodb-server.ts
var DynamoDBServer = class {
	...
	async listen(port) {
		if (this.server || this.javaServer) throw new Error("Server is already running");
		...
		if (this.config.engine === "java") {
			...
			const serverOrPromise = createServer$1(this.store, listenPort, this.config.hostname);
			if (serverOrPromise instanceof Promise) this.server = await serverOrPromise;
			else this.server = serverOrPromise;
	...
	getClient() {
		if (!this.client) this.client = new DynamoDBClient({
	...
	getDocumentClient() {
		if (!this.documentClient) this.documentClient = DynamoDBDocumentClient.from(this.getClient(), { marshallOptions: { removeUndefinedValues: true } });
	...
	*/
	advanceTime(ms) {
		if (this.config.engine === "java") throw new Error("advanceTime is not supported with the Java engine");
	...
	*/
	setTime(timestamp) {
		if (this.config.engine === "java") throw new Error("setTime is not supported with the Java engine");
	...
	*/
	getTime() {
		if (this.config.engine === "java") throw new Error("getTime is not supported with the Java engine");
	...
	*/
	onStreamRecord(tableName, callback) {
		if (this.config.engine === "java") throw new Error("onStreamRecord is not supported with the Java engine");
		return this.store.getTable(tableName).onStreamRecord(callback);
	}
	/**
	...
	*/
	reset() {
		if (this.config.engine === "java") throw new Error("reset is not supported with the Java engine");
		this.store.clear();
		this.clock.reset();
	...
	*/
	getTableStore() {
		if (this.config.engine === "java") throw new Error("getTableStore is not supported with the Java engine");
		return this.store;
	}
};
//#endregion
export { ConditionalCheckFailedException, DynamoDBError, DynamoDBServer, IdempotentParameterMismatchException, InternalServerError, ItemCollectionSizeLimitExceededException, ProvisionedThroughputExceededException, ResourceInUseException, ResourceNotFoundException, SerializationException, TransactionCanceledException, TransactionConflictException, ValidationException, VirtualClock };
