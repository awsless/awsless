...
export const createDevContext = (props: {
	...
	// only point at one endpoint, so they share one lazy server.
	let dynamo: Promise<DynamoDBServer> | undefined
	...
	const useDynamo = () => {
		dynamo ??= (async () => {
			...
			env['AWS_ENDPOINT_URL_DYNAMODB'] = `http://127.0.0.1:${server.port}`
	...
	// can register their notification rules before traffic flows.
	...
	const useStore = () => {
		store ??= (async () => {
			...
			env['AWS_ENDPOINT_URL_S3'] = `http://127.0.0.1:${port}`
