...
export const startDev = async (props: {
	...
	const dev = createDevContext({ appConfig, stackConfigs, appId, routerPorts, log })
	...
	const blocked = createBlockedServer({ onLog: log })
	const blockedPort = await blocked.listen()
	...
	// into the worker.
	const lambda = createLambdaServer({ functionName: bundleName })
	const lambdaPort = await lambda.listen()
