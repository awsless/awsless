...
export const siteOnDev = async (ctx: DevContext) => {
	for (const stackConfig of ctx.stackConfigs) {
		for (const [id, props] of Object.entries(stackConfig.sites ?? {})) {
			...
			if (props.dev) {
				const port = props.dev.port ?? (await findFreePort())
				...
				ctx.registerServer({
					...
					async start({ env }) {
						...
						// like "awsless bind --local". Its output stays out of
						// the cli log: every line streams to the dashboard
						// panel & the last lines only surface in the terminal
						// when the dev server dies unexpectedly.
						...
						child.on('exit', code => {
							// A signal exit (code null) is a shutdown, like the
							// terminal group SIGINT of a ctrl-c - only a real
							// non-zero exit is a crash. The boot log prop only
							// updates the transient spinner message, so the
							// crash logs directly.
							if (!stopping && code !== null && code !== 0) {
								log.error(`The site "${id}" dev server exited with code ${code}:\n${tail.join('\n')}`)
								ctx.emitEvent(channel, { date: Date.now(), line: `Exited with code ${code}` })
							}
						})
					},
					async stop() {
