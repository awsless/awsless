...
__export(src_exports, {
  App: () => App,
  AppError: () => AppError,
  DynamoLockProvider: () => DynamoLockProvider,
  ...
  createResourceMeta: () => createResourceMeta,
...
var DependencyGraph = class {
  ...
  run() {
    return Promise.allSettled(Object.values((0, import_promise_dag.run)(this.graph)));
  }
};
...
var createPluginServer = (props) => {
  return new Promise((resolve2, reject) => {
    const process2 = (0, import_node_child_process.spawn)(`${props.file}`, ["-debug"]);
    debug9("init");
...
var groupByNamespace = (resources, minLevel, maxLevel) => {
  ...
  for (const type of Object.keys(resources)) {
    ...
    while (count--) {
      const ns = (0, import_change_case.camelCase)(names.shift());
      if (!current[ns]) {
        current[ns] = {};
      }
      current = current[ns];
    }
    const name = (0, import_change_case.pascalCase)(names.join("_"));
    current[name] = type;
  }
  return grouped;
};
...
`);
  ...
  const renderResource = (name, type, indent) => {
    const tabs = `	`.repeat(indent);
    const propName = (0, import_change_case.pascalCase)(name);
    const className = (0, import_change_case.pascalCase)(type);
...
var generateResourceInputTypes = (type, prop) => {
  const code = [
    `type ${(0, import_change_case.pascalCase)(type)}Props = `,
    generateValue(prop, {
      depth: 0,
      indent: 1,
...
var generateResourceOutputTypes = (type, prop) => {
  const code = [
    `type ${(0, import_change_case.pascalCase)(type)} = `,
    generateValue(prop, {
      indent: 1,
      depth: 0,
...
var TerraformProvider = class {
  ...
  async generateTypes(dir) {
    ...
    await (0, import_promises5.writeFile)((0, import_node_path4.join)(dir, `${this.id}.types.d.ts`), types);
    await this.destroy();
  }
};
...
var tf = createRecursiveProxy(
  (names, parent, id, input, config) => {
    const type = (0, import_change_case2.snakeCase)(names.join("_"));
