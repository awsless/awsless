...
describe('Update Expression', () => {
	...
	assert(
		'SET #n1 = :v1',
		(exp) => exp.update('id').set(1)
	)

	assert(
		'SET #n1 = if_not_exists( #n1 , :v1 )',
		(exp) => exp.update('id').setIfNotExists(1)
	)

	assert(
		'REMOVE #n1',
		(exp) => exp.update('id').del()
	)

	assert(
		'SET #n1 = if_not_exists( #n1 , :v1 ) + :v2',
		(exp) => exp.update('id').incr()
	)

	assert(
		'SET #n1 = if_not_exists( #n1 , :v1 ) - :v2',
		(exp) => exp.update('id').decr()
	)

	assert(
		'ADD #n1 :v1',
		(exp) => exp.update('id').append(1)
	)

	assert(
		'DELETE #n1 :v1',
		(exp) => exp.update('id').remove(1)
	)
	...
	assert(
		...
		(exp) => exp
			...
			.update('id').decr()
			.update('id').append(1)
			.update('name').append('Jack')
			.update('name').remove('Jack')
	)
