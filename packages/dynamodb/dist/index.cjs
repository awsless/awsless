...
let _awsless_dynamodb_server = require("@awsless/dynamodb-server");
...
let _smithy_node_http_handler = require("@smithy/node-http-handler");
//#region src/table.ts
var Table = class {
	name;
	hash;
...
const unknown = (opts) => createSchema({
	...
	unmarshall(value) {
		...
		return (0, _aws_sdk_util_dynamodb.unmarshall)({ value }, opts?.unmarshall).value;
	},
	validateInput: () => true,
	validateOutput: () => true
});
...
const bigfloat = ({ precision = _awsless_big_float.PRECISION } = {}) => createSchema({
	name: "bigfloat",
	type: "N",
	marshall: (value) => ({ N: (0, _awsless_big_float.floor)(value, precision).toString() }),
	unmarshall: (value) => (0, _awsless_big_float.parse)(value.N),
	validateInput: (value) => value instanceof _awsless_big_float.BigFloat,
	validateOutput: (value) => typeof value === "object" && "N" in value && typeof value.N === "string"
});
//#endregion
...
const uint8array = () => createSchema({
	...
	validateOutput: (value) => typeof value === "object" && "B" in value && (0, node_util_types.isUint8Array)(value.B)
});
//#endregion
...
const json = () => createSchema({
	name: "json",
	type: "S",
	marshall: (value) => ({ S: (0, _awsless_json.stringify)(value) }),
	unmarshall: (value) => (0, _awsless_json.parse)(value.S),
	validateInput: () => true,
...
const migrate = (client, tables) => {
	return Promise.all([tables].flat().map((table) => {
		...
		return client.send(new _aws_sdk_client_dynamodb.CreateTableCommand({
			...table,
...
const dynamoDBClient = /* @__PURE__ */ (0, _awsless_utils.globalClient)(() => {
	return new _aws_sdk_client_dynamodb.DynamoDBClient({
		maxAttempts: 2,
		requestHandler: new _smithy_node_http_handler.NodeHttpHandler({
			connectionTimeout: 3e3,
			requestTimeout: 3e3
		})
	});
});
const dynamoDBDocumentClient = /* @__PURE__ */ (0, _awsless_utils.globalClient)(() => {
	return _aws_sdk_lib_dynamodb.DynamoDBDocumentClient.from(dynamoDBClient(), { marshallOptions: { removeUndefinedValues: true } });
});
...
const putItems = (table, items, options = {}) => {
	...
	return thenable(async () => {
		...
		while (unprocessedItems.length) {
			const command = new _aws_sdk_client_dynamodb.BatchWriteItemCommand({ RequestItems: { [table.name]: unprocessedItems.splice(0, 25) } });
...
const getItem = (table, key, options = {}) => {
	...
	const client = getClient(options);
	const command = new _aws_sdk_client_dynamodb.GetItemCommand({
		TableName: table.name,
...
const pipeStream = (streams, command, send) => {
	if (command instanceof _aws_sdk_client_dynamodb.PutItemCommand) return pipeToTable({
		streams,
		command,
		send,
	...
	if (command instanceof _aws_sdk_client_dynamodb.UpdateItemCommand || command instanceof _aws_sdk_client_dynamodb.DeleteItemCommand) return pipeToTable({
		streams,
		command,
		send,
		getKey: (command, table) => {
	...
	if (command instanceof _aws_sdk_client_dynamodb.BatchWriteItemCommand) return pipeToTables({
		command,
		send,
		getEntries: (command) => {
	...
	if (command instanceof _aws_sdk_client_dynamodb.TransactWriteItemsCommand) return pipeToTables({
		command,
		send,
		getEntries: (command) => {
...
const mockDynamoDB = (configOrServer) => {
	let server;
	if (configOrServer instanceof _awsless_dynamodb_server.DynamoDBServer) server = configOrServer;
	else {
		server = new _awsless_dynamodb_server.DynamoDBServer({ engine: configOrServer.engine === "correctness" ? "java" : "memory" });
		if (typeof beforeAll !== "undefined") beforeAll(async () => {
			...
			const dbMock = (0, aws_sdk_vitest_mock.mockClient)(_aws_sdk_client_dynamodb.DynamoDBClient);
			dbMock.on(_aws_sdk_client_dynamodb.CreateTableCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.CreateTableCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.ListTablesCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.ListTablesCommand(input ?? {})));
			dbMock.on(_aws_sdk_client_dynamodb.GetItemCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.GetItemCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.PutItemCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.PutItemCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.DeleteItemCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.DeleteItemCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.UpdateItemCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.UpdateItemCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.QueryCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.QueryCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.ScanCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.ScanCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.BatchGetItemCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.BatchGetItemCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.BatchWriteItemCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.BatchWriteItemCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.TransactGetItemsCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.TransactGetItemsCommand(input)));
			dbMock.on(_aws_sdk_client_dynamodb.TransactWriteItemsCommand).callsFake((input) => clientSend(new _aws_sdk_client_dynamodb.TransactWriteItemsCommand(input)));
			const docMock = (0, aws_sdk_vitest_mock.mockClient)(_aws_sdk_lib_dynamodb.DynamoDBDocumentClient);
			docMock.on(_aws_sdk_lib_dynamodb.GetCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.GetCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.PutCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.PutCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.DeleteCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.DeleteCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.UpdateCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.UpdateCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.QueryCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.QueryCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.ScanCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.ScanCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.BatchGetCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.BatchGetCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.BatchWriteCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.BatchWriteCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.TransactGetCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.TransactGetCommand(input)));
			docMock.on(_aws_sdk_lib_dynamodb.TransactWriteCommand).callsFake((input) => documentClientSend(new _aws_sdk_lib_dynamodb.TransactWriteCommand(input)));
...
_aws_sdk_client_dynamodb.TransactionCanceledException.prototype.cancellationReasonAt = function(index) {
	const reason = (this.CancellationReasons ?? [])[index];
	if (!reason) throw new Error(`Cancellation reason index is out of bounds: ${index}`);
	return reason.Code;
};
_aws_sdk_client_dynamodb.TransactionCanceledException.prototype.conditionFailedAt = function(index) {
	return this.cancellationReasonAt(index) === "ConditionalCheckFailed";
};
_aws_sdk_client_dynamodb.TransactionCanceledException.prototype.conflictAt = function(index) {
	return this.cancellationReasonAt(index) === "TransactionConflict";
};
_aws_sdk_client_dynamodb.TransactionCanceledException.prototype.validationErrorAt = function(index) {
...
const putItem = (table, item, options = {}) => {
	const client = getClient(options);
	const attrs = new ExpressionAttributes(table);
	const command = new _aws_sdk_client_dynamodb.PutItemCommand({
		TableName: table.name,
...
const updateItem = (table, key, options) => {
	...
	const command = new _aws_sdk_client_dynamodb.UpdateItemCommand({
		TableName: table.name,
		Key: table.marshall(key),
...
const deleteItem = (table, key, options = {}) => {
	const client = getClient(options);
	const attrs = new ExpressionAttributes(table);
	const command = new _aws_sdk_client_dynamodb.DeleteItemCommand({
		TableName: table.name,
		Key: table.marshall(key),
		ConditionExpression: buildConditionExpression(attrs, options.when),
		ReturnValues: options.return,
...
const getItems = (table, keys, options = { filterNonExistentItems: false }) => {
	...
	return thenable(async () => {
		...
		while (unprocessedKeys.length) {
			const command = new _aws_sdk_client_dynamodb.BatchGetItemCommand({ RequestItems: { [table.name]: {
...
const query = (table, key, options = {}) => {
	...
	const execute = async (cursor, limit) => {
		...
		const attrs = new ExpressionAttributes(table);
		const command = new _aws_sdk_client_dynamodb.QueryCommand({
			TableName: table.name,
...
const scan = (table, options = {}) => {
	...
	const execute = async (cursor, limit) => {
		const attrs = new ExpressionAttributes(table);
		const command = new _aws_sdk_client_dynamodb.ScanCommand({
			TableName: table.name,
			ConsistentRead: options.consistentRead,
			Limit: limit ?? options.limit ?? 10,
			ExclusiveStartKey: fromCursorString(cursor),
...
const transactWrite = async (items, options = {}) => {
	const client = getClient(options);
	const command = new _aws_sdk_client_dynamodb.TransactWriteItemsCommand({
...
const transactRead = async (items, options = {}) => {
	...
	const command = new _aws_sdk_client_dynamodb.TransactGetItemsCommand({ TransactItems: transactItems.map((item) => item.input) });
	return ((await getClient(options).send(command)).Responses ?? []).map((res, i) => {
		if (res.Item) return transactItems[i].unmarshall(res.Item);
	});
};
//#endregion
Object.defineProperty(exports, "BatchGetItemCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.BatchGetItemCommand;
	}
});
Object.defineProperty(exports, "BatchWriteItemCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.BatchWriteItemCommand;
	}
});
Object.defineProperty(exports, "ConditionalCheckFailedException", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.ConditionalCheckFailedException;
	}
});
Object.defineProperty(exports, "DeleteItemCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.DeleteItemCommand;
	}
});
Object.defineProperty(exports, "DynamoDBClient", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.DynamoDBClient;
	}
});
Object.defineProperty(exports, "DynamoDBDocumentClient", {
	enumerable: true,
	get: function() {
		return _aws_sdk_lib_dynamodb.DynamoDBDocumentClient;
	}
});
Object.defineProperty(exports, "DynamoDBServer", {
	enumerable: true,
	get: function() {
		return _awsless_dynamodb_server.DynamoDBServer;
	}
});
Object.defineProperty(exports, "DynamoDBServiceException", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.DynamoDBServiceException;
	}
});
exports.Fluent = Fluent;
Object.defineProperty(exports, "GetItemCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.GetItemCommand;
	}
});
Object.defineProperty(exports, "PutItemCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.PutItemCommand;
	}
});
Object.defineProperty(exports, "QueryCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.QueryCommand;
	}
});
Object.defineProperty(exports, "ScanCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.ScanCommand;
	}
});
exports.Table = Table;
Object.defineProperty(exports, "TransactGetItemsCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.TransactGetItemsCommand;
	}
});
Object.defineProperty(exports, "TransactWriteItemsCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.TransactWriteItemsCommand;
	}
});
Object.defineProperty(exports, "TransactionCanceledException", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.TransactionCanceledException;
	}
});
Object.defineProperty(exports, "TransactionConflictException", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.TransactionConflictException;
	}
});
Object.defineProperty(exports, "TransactionInProgressException", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.TransactionInProgressException;
	}
});
Object.defineProperty(exports, "UpdateItemCommand", {
	enumerable: true,
	get: function() {
		return _aws_sdk_client_dynamodb.UpdateItemCommand;
	}
});
exports.any = any;
exports.array = array;
...
exports.set = set;
exports.streamTable = streamTable;
