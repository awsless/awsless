...
import { BatchWriteItemCommand, DeleteItemCommand, PutItemCommand, TransactWriteItemsCommand, UpdateItemCommand } from "@aws-sdk/client-dynamodb"
...
export const streamTable = <T extends AnyTableDefinition>(table:T, fn: (payload: StreamRequest<T>) => void): Stream<AnyTableDefinition> => {
...
export const pipeStream = (streams: Stream<AnyTableDefinition>[], command:any, send:<T>() => T) => {
	if(command instanceof PutItemCommand) {
		return pipeToTable({ streams, command, send, getKey: (command, table) => {
	...
	if(command instanceof BatchWriteItemCommand) {
		return pipeToTables({ command, send, getEntries: (command) => {
...
const pipeToTables = async <Command>(
	{ command, send, getEntries }: {
		command: Command, send:<T>() => T,
		getEntries: (command: Command) => Array<Stream<AnyTableDefinition> & {
			items: Array<{
				key: any,
				OldImage?: any
				NewImage?: any
