...
const luaString = (value) => (0, fengari.to_luastring)(value);
...
var LuaRuntime = class LuaRuntime {
	...
	constructor(engine) {
		...
		for (const [name, open] of [
			["_G", fengari.lualib.luaopen_base],
			["string", fengari.lualib.luaopen_string],
			["table", fengari.lualib.luaopen_table],
			["math", fengari.lualib.luaopen_math]
		]) {
			fengari.lauxlib.luaL_requiref(L, luaString(name), open, 1);
		...
		for (const name of [
			...
			fengari.lua.lua_setglobal(L, luaString(name));
		}
		fengari.lua.lua_atnativeerror(L, (L) => {
			const err = fengari.lua.lua_touserdata(L, 1);
			fengari.lua.lua_pushstring(L, luaString(err instanceof Error ? err.message : String(err)));
			return 1;
		});
		this.registerRedisLibrary();
		this.registerCjsonLibrary();
		fengari.lauxlib.luaL_requiref(L, luaString("debug"), fengari.lualib.luaopen_debug, 0);
		this.compile(ERROR_HANDLER, "redis_error_handler");
		fengari.lua.lua_pushvalue(L, -2);
		fengari.lua.lua_call(L, 1, 1);
		this.errorHandlerRef = fengari.lauxlib.luaL_ref(L, fengari.lua.LUA_REGISTRYINDEX);
		fengari.lua.lua_pop(L, 1);
		this.compile(PROTECT_GLOBALS, "protect_globals");
		fengari.lua.lua_call(L, 0, 0);
	}
	static sha1(body) {
		return (0, node_crypto.createHash)("sha1").update(Buffer.from(body, "latin1")).digest("hex");
	}
	exists(sha) {
	...
	load(body) {
		...
		if (!this.scripts.has(sha)) {
			...
			const ref = fengari.lauxlib.luaL_ref(this.L, fengari.lua.LUA_REGISTRYINDEX);
	...
	flush() {
		for (const script of this.scripts.values()) fengari.lauxlib.luaL_unref(this.L, fengari.lua.LUA_REGISTRYINDEX, script.ref);
		this.scripts.clear();
	}
	run(sha, keys, argv, conn, readOnly) {
		...
		const L = this.L;
		const base = fengari.lua.lua_gettop(L);
		this.setGlobalArray("KEYS", keys);
		this.setGlobalArray("ARGV", argv);
		fengari.lua.lua_rawgeti(L, fengari.lua.LUA_REGISTRYINDEX, this.errorHandlerRef);
		fengari.lua.lua_rawgeti(L, fengari.lua.LUA_REGISTRYINDEX, script.ref);
		this.current = {
			...conn,
			multi: null,
			watches: [],
		...
		try {
			if (fengari.lua.lua_pcall(L, 0, 1, base + 1) !== fengari.lua.LUA_OK) throw new RedisError(this.errorMessage(-1, sha));
			return this.toReply(-1);
		} finally {
			this.current = null;
			fengari.lua.lua_settop(L, base);
		}
	}
	compile(body, name) {
		const bytes = toBytes(body);
		if (fengari.lauxlib.luaL_loadbuffer(this.L, bytes, bytes.length, luaString(name)) !== fengari.lua.LUA_OK) {
			const message = fromBytes(fengari.lua.lua_tolstring(this.L, -1));
	...
	setGlobalArray(name, values) {
		const L = this.L;
		fengari.lua.lua_pushglobaltable(L);
		fengari.lua.lua_pushstring(L, luaString(name));
		fengari.lua.lua_newtable(L);
		values.forEach((value, i) => {
			fengari.lua.lua_pushstring(L, toBytes(value));
			fengari.lua.lua_rawseti(L, -2, i + 1);
		});
		fengari.lua.lua_rawset(L, -3);
		fengari.lua.lua_pop(L, 1);
	}
	errorMessage(index, sha) {
		const L = this.L;
		const abs = fengari.lua.lua_absindex(L, index);
		if (fengari.lua.lua_type(L, abs) !== fengari.lua.LUA_TTABLE) {
			const text = fengari.lua.lua_tolstring(L, abs);
			return `ERR Error running script ${sha}: ${text === null ? "execution failure" : fromBytes(text)}`;
		}
		const field = (name) => {
			fengari.lua.lua_getfield(L, abs, luaString(name));
			const value = fengari.lua.lua_tolstring(L, -1);
			fengari.lua.lua_pop(L, 1);
	...
	pushReply(reply) {
		...
		switch (reply.type) {
			case "status":
				fengari.lua.lua_newtable(L);
				fengari.lua.lua_pushstring(L, toBytes(reply.value));
				fengari.lua.lua_setfield(L, -2, luaString("ok"));
				break;
			case "error":
				fengari.lua.lua_newtable(L);
				fengari.lua.lua_pushstring(L, toBytes(reply.value));
				fengari.lua.lua_setfield(L, -2, luaString("err"));
				break;
			case "int": {
				const n = Number(reply.value);
				if (Number.isInteger(n) && n >= -2147483648 && n <= 2147483647) fengari.lua.lua_pushinteger(L, n);
				else fengari.lua.lua_pushnumber(L, n);
				break;
			}
			case "bulk":
				if (reply.value === null) fengari.lua.lua_pushboolean(L, false);
				else fengari.lua.lua_pushstring(L, toBytes(reply.value));
				break;
			case "array":
				if (reply.value === null) fengari.lua.lua_pushboolean(L, false);
				else {
				...
				break;
			default: fengari.lua.lua_pushboolean(L, false);
		}
	}
	toReply(index) {
		const L = this.L;
		const abs = fengari.lua.lua_absindex(L, index);
		const type = fengari.lua.lua_type(L, abs);
		if (type === fengari.lua.LUA_TSTRING) return bulk(fromBytes(fengari.lua.lua_tolstring(L, abs)));
		if (type === fengari.lua.LUA_TBOOLEAN) return fengari.lua.lua_toboolean(L, abs) ? int(1) : NIL;
		if (type === fengari.lua.LUA_TNUMBER) {
			const n = Math.trunc(fengari.lua.lua_tonumber(L, abs));
			return int(Number.isSafeInteger(n) ? n : BigInt(n));
		}
		if (type !== fengari.lua.LUA_TTABLE) return NIL;
		fengari.lua.lua_getfield(L, abs, luaString("err"));
		if (fengari.lua.lua_type(L, -1) === fengari.lua.LUA_TSTRING) {
			const message = fromBytes(fengari.lua.lua_tolstring(L, -1)).replace(/\r?\n/g, " ");
			fengari.lua.lua_pop(L, 1);
			return error(message);
		}
		fengari.lua.lua_pop(L, 1);
		fengari.lua.lua_getfield(L, abs, luaString("ok"));
		if (fengari.lua.lua_type(L, -1) === fengari.lua.LUA_TSTRING) {
			const message = fromBytes(fengari.lua.lua_tolstring(L, -1)).replace(/\r?\n/g, " ");
			fengari.lua.lua_pop(L, 1);
		...
		const items = [];
		for (let i = 1;; i++) {
			fengari.lua.lua_rawgeti(L, abs, i);
			if (fengari.lua.lua_type(L, -1) === fengari.lua.LUA_TNIL) {
				fengari.lua.lua_pop(L, 1);
				break;
			}
	...
	raiseError(message) {
		const L = this.L;
		fengari.lua.lua_newtable(L);
		fengari.lua.lua_pushstring(L, toBytes(message));
		fengari.lua.lua_setfield(L, -2, luaString("err"));
		fengari.lauxlib.luaL_where(L, 1);
		const where = fromBytes(fengari.lua.lua_tolstring(L, -1));
		fengari.lua.lua_pop(L, 1);
		const line = /:(\d+):/.exec(where)?.[1];
		if (line) {
			fengari.lua.lua_pushstring(L, luaString("@user_script"));
			fengari.lua.lua_setfield(L, -2, luaString("source"));
			fengari.lua.lua_pushstring(L, luaString(line));
			fengari.lua.lua_setfield(L, -2, luaString("line"));
		}
	...
	call(raise) {
		...
		const argv = [];
		for (let i = 1; i <= n; i++) {
			const type = fengari.lua.lua_type(L, i);
			if (type === fengari.lua.LUA_TNUMBER) argv.push(formatLuaNumber(L, i));
			else if (type === fengari.lua.LUA_TSTRING) argv.push(fromBytes(fengari.lua.lua_tolstring(L, i)));
	...
	registerRedisLibrary() {
		...
		const register = (name, fn) => {
			fengari.lua.lua_pushjsfunction(L, fn);
			fengari.lua.lua_setfield(L, -2, luaString(name));
		};
		register("call", () => this.call(true));
		register("pcall", () => this.call(false));
		register("sha1hex", (L) => {
			if (fengari.lua.lua_gettop(L) !== 1) return this.raiseError("ERR wrong number of arguments");
			const text = fengari.lua.lua_tolstring(L, 1);
			fengari.lua.lua_pushstring(L, luaString(LuaRuntime.sha1(fromBytes(text))));
			return 1;
		});
		register("error_reply", (L) => {
			const text = fengari.lua.lua_tolstring(L, 1);
			if (fengari.lua.lua_type(L, 1) !== fengari.lua.LUA_TSTRING || text === null) return this.raiseError("ERR wrong number or type of arguments");
		...
		register("log", (L) => {
			if (fengari.lua.lua_gettop(L) < 2) return this.raiseError("ERR redis.log() requires two arguments or more.");
			if (fengari.lua.lua_type(L, 1) !== fengari.lua.LUA_TNUMBER) return this.raiseError("ERR First argument must be a number (log level).");
			return 0;
		});
		register("replicate_commands", (L) => {
			fengari.lua.lua_pushboolean(L, 1);
			return 1;
		});
		register("setresp", (L) => {
			if (fengari.lua.lua_tonumber(L, 1) === 2) return 0;
		...
		for (const [name, value] of [
			...
			fengari.lua.lua_pushinteger(L, value);
			fengari.lua.lua_setfield(L, -2, luaString(name));
		}
		fengari.lua.lua_pushstring(L, luaString(REDIS_VERSION));
		fengari.lua.lua_setfield(L, -2, luaString("REDIS_VERSION"));
		fengari.lua.lua_setglobal(L, luaString("redis"));
	}
	registerCjsonLibrary() {
		const L = this.L;
		fengari.lua.lua_newtable(L);
		fengari.lua.lua_pushjsfunction(L, (L) => {
			if (fengari.lua.lua_gettop(L) !== 1) return this.raiseError("ERR bad argument #1 to encode (expected one argument)");
			const json = JSON.stringify(this.toJson(1, 0));
			fengari.lua.lua_pushstring(L, Buffer.from(json, "utf8"));
			return 1;
		});
		fengari.lua.lua_setfield(L, -2, luaString("encode"));
		fengari.lua.lua_pushjsfunction(L, (L) => {
			const text = fengari.lua.lua_tolstring(L, 1);
			if (fengari.lua.lua_type(L, 1) !== fengari.lua.LUA_TSTRING || text === null) return this.raiseError("ERR bad argument #1 to decode (string expected)");
			let value;
			try {
		...
		fengari.lua.lua_setfield(L, -2, luaString("decode"));
		fengari.lua.lua_pushlightuserdata(L, NULL);
		fengari.lua.lua_setfield(L, -2, luaString("null"));
		fengari.lua.lua_setglobal(L, luaString("cjson"));
	}
	pushJson(value) {
		const L = this.L;
		if (value === null || value === void 0) fengari.lua.lua_pushlightuserdata(L, NULL);
		else if (typeof value === "boolean") fengari.lua.lua_pushboolean(L, value);
		else if (typeof value === "number") {
			if (Number.isInteger(value) && value >= -2147483648 && value <= 2147483647) fengari.lua.lua_pushinteger(L, value);
			else fengari.lua.lua_pushnumber(L, value);
		} else if (typeof value === "string") fengari.lua.lua_pushstring(L, Buffer.from(value, "utf8"));
			...
			for (const [key, item] of Object.entries(value)) {
				fengari.lua.lua_pushstring(L, Buffer.from(key, "utf8"));
				this.pushJson(item);
				fengari.lua.lua_rawset(L, -3);
			}
		}
	...
	toJson(index, depth) {
		...
		if (type === fengari.lua.LUA_TNIL || type === fengari.lua.LUA_TLIGHTUSERDATA) return null;
		if (type === fengari.lua.LUA_TBOOLEAN) return fengari.lua.lua_toboolean(L, abs);
		if (type === fengari.lua.LUA_TNUMBER) {
			const n = fengari.lua.lua_tonumber(L, abs);
			if (!Number.isFinite(n)) return this.raiseError("ERR Cannot serialise number: must not be NaN or Inf");
			return fengari.lua.lua_isinteger(L, abs) ? n : Number(formatG(n, 14));
		}
		if (type === fengari.lua.LUA_TSTRING) {
			const bytes = fengari.lua.lua_tolstring(L, abs);
			return Buffer.from(bytes.buffer, bytes.byteOffset, bytes.length).toString("utf8");
		}
		if (type !== fengari.lua.LUA_TTABLE) return this.raiseError(`ERR Cannot serialise ${type === fengari.lua.LUA_TFUNCTION ? "function" : "userdata"}: type not supported`);
		const object = {};
		const array = [];
		let isArray = true;
		let max = 0;
		fengari.lua.lua_pushnil(L);
		while (fengari.lua.lua_next(L, abs) !== 0) {
			const value = this.toJson(-1, depth + 1);
			if (fengari.lua.lua_type(L, -2) === fengari.lua.LUA_TNUMBER) {
				const key = fengari.lua.lua_tonumber(L, -2);
				...
				isArray = false;
				const key = fengari.lua.lua_tolstring(L, -2);
...
const formatLuaNumber = (L, index) => {
	if (fengari.lua.lua_isinteger(L, index)) return String(fengari.lua.lua_tointeger(L, index));
	const n = fengari.lua.lua_tonumber(L, index);
...
var MemoryRedisServer = class {
	...
	async listen(port = this.defaultPort) {
		...
		const server = (0, node_net.createServer)((socket) => this.handle(socket));
		this.server = server;
		await new Promise((resolve, reject) => {
			const onError = (err) => {
				this.server = void 0;
...
//#endregion
exports.MemoryRedisServer = MemoryRedisServer;
exports.ProtocolError = ProtocolError;
exports.RealRedisServer = RealRedisServer;
